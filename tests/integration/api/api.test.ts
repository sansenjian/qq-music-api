...
describe('API Integration Tests', () => {
	...
	describe('GET /getSearchByKey', () => {
		test('should search with query param', async () => {
			const response = await request(callback).get('/getSearchByKey').query({ key: 'test' }).expect(200);
			expectSuccessResponse(response.body);
		});
	...
	describe('GET /getLyric', () => {
		...
		test('should fallback to musicu lyric api when primary lyric api returns negative code', async () => {
			...
			(global as unknown as { fetch: Mock }).fetch = vi.fn().mockResolvedValueOnce(
				createFetchResponse({
					text: JSON.stringify({
						req_0: {
							data: {
			...
			expect((global as unknown as { fetch: Mock }).fetch).toHaveBeenCalledWith(
		...
		test('should keep primary payload when musicu fallback request fails', async () => {
			...
			(global as unknown as { fetch: Mock }).fetch = vi.fn().mockRejectedValueOnce(new Error('socket hang up'));
	...
	describe('GET /getMusicPlay', () => {
		...
		test('should return 502 and propagate upstream error when service fails', async () => {
			...
			const response = await request(callback).get('/getMusicPlay').query({ songmid: 'test-songmid' }).expect(502);
		...
		test('should return play url map when upstream returns purl', async () => {
			...
			const response = await request(callback).get('/getMusicPlay').query({ songmid: 'test-mid-1' }).expect(200);
		...
		test.each([
			['comma-separated', '/getMusicPlay', { songmid: 'a,b,c' }],
			['repeated query keys', '/getMusicPlay?songmid=a&songmid=b&songmid=c', undefined],
		])('should handle %s songmid and include missing entries', async (_label, path, query) => {
			mockService.mockResolvedValueOnce({
				data: {
					req_0: {
						data: {
							sip: ['https://isure.stream.qqmusic.qq.com/'],
							midurlinfo: [
								{ songmid: 'a', purl: 'M500aa.mp3' },
								{ songmid: 'b', purl: 'M500bb.mp3' },
							],
						},
					},
				},
			...
			const req = request(callback).get(path);
			const response = query ? await req.query(query).expect(200) : await req.expect(200);
		...
		test('should build fallback url when purl is empty but filename and vkey exist', async () => {
			...
			const response = await request(callback).get('/getMusicPlay').query({ songmid: 'test-mid-2' }).expect(200);
		...
		test('should keep url empty and set error when purl is empty and filename/vkey are missing', async () => {
			...
			const response = await request(callback).get('/getMusicPlay').query({ songmid: 'test-mid-empty' }).expect(200);
	...
	describe('POST /batchGetSongInfo', () => {
		...
		test('should return empty array when songs is missing', async () => {
			const response = await request(callback).post('/batchGetSongInfo').send({}).expect(200);
		...
		test('should use empty string as default song_id when it is omitted', async () => {
			...
			const firstCallConfig = mockService.mock.calls[0][0] as {
				params: { data: { songinfo: { param: { song_id: string } } } };
			};
	...
	describe('GET /user/getUserPlaylists', () => {
		...
		test('should return playlists when upstream payload contains playlist field', async () => {
			...
			const response = await request(callback).get('/user/getUserPlaylists').query({ uin: '123456789' }).expect(200);
	...
	describe('GET /user/getUserAvatar', () => {
		...
		test.each([
			...
			const response = await request(callback).get('/user/getUserAvatar').query({ uin: '123456789', size }).expect(400);
		...
		test('should return avatar url when uin is provided', async () => {
			...
			expectSuccessResponse(response.body);
			expect(response.body.response.data.avatarUrl).toBe('https://q.qlogo.cn/headimg_dl?dst_uin=123456789&spec=140');
		});
		...
		test('should prefer k over uin when both are provided', async () => {
			...
			expectSuccessResponse(response.body);
			expect(response.body.response.data.avatarUrl).toBe('https://thirdqq.qlogo.cn/g?b=sdk&k=mock-k&s=640');
		});
	});
	...
	describe('GET /getMvPlay', () => {
		...
		test('should return grouped playLists when upstream returns nested freeflow urls', async () => {
			...
			const response = await request(callback).get('/getMvPlay').query({ vid: 'testVid' }).expect(200);

			expect((response.body as AnyRecord).response).toMatchObject({
				playLists: {
		...
		test('should return 502 when upstream mv url payload is empty', async () => {
			...
			const response = await request(callback).get('/getMvPlay').query({ vid: 'testVid' }).expect(502);

			expect((response.body as AnyRecord).response).toEqual({
				data: null,
	...
	describe('POST /batchGetSongLists', () => {
		test('should use default categoryIds when request body omits them', async () => {
			...
			const response = await request(callback).post('/batchGetSongLists').send({}).expect(200);
	...
	describe('QQ QR login endpoints', () => {
		...
		test('POST /checkQQLoginQr should return session on success', async () => {
			...
			expect(global.userInfo).toBe(originalUserInfo);
		});
		...
		test('POST /checkQQLoginQr should return 504 and 登录检查超时 on fetch timeout', async () => {
			...
			(global as unknown as { fetch: Mock }).fetch = vi.fn().mockRejectedValueOnce(abortError);
		...
		test('GET /getQQLoginQr should map upstream failure to 502', async () => {
			(global as unknown as { fetch: Mock }).fetch = vi.fn().mockRejectedValueOnce(new Error('network timeout'));
